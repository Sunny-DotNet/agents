...
`templates/` contains the source of truth: one agent template per `*.role.json` file, for example `templates/sophie.role.json`. `scripts/` holds the repository automation: `validate-templates.ps1` validates template shape and dictionary usage, and `generate-index.ps1` rebuilds `index.json`. `job_titles.json` is the shared job title dictionary. `index.html` powers the GitHub Pages viewer. `.github/workflows/` runs validation, regenerates `index.json` on `main`, and deploys Pages.
...
Regenerate the published index after changing `templates/` or `job_titles.json`:
...
Use 2-space indentation in JSON files and keep keys stable unless the schema changes. Template files should use the `templates/<name>.role.json` pattern. Template `schema` must remain `openstaff.role-sync.v2`. `jobTitle` must match an English alias from `job_titles.json`, whose entries are sorted by `key`. Keep `mcps` and `skills` as arrays of objects with a `key` field.

## Testing Guidelines
The validation script is the main test gate; there is no separate unit test suite in this repo. Any change to a template, `job_titles.json`, or validation logic should pass `validate-templates.ps1`. If you modify templates or the job title dictionary, regenerate `index.json` and confirm the diff is expected.
